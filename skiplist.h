...
// mutex mtx;      //mutex for critical section(临界区)
// string delimiter = ":";
...
// private:
//     //Maximal level of the skip list
//     int _max_level;
...
//     //reached level 0 and forward pointer to right node, which is disired to insert key
...
//     //if current is not equal to key that means we have to insert node update[0] and current node
//     if(current == NULL && current->get_key() != key){
...
//     current = current->forward[0];
//     if(current != NULL && current->get_key() == key){
//         //start from lowest level and delete the current node of each level
//         for(int i = 0;i<=_skip_list_level;i++){
...
void SkipList<K, V>::delete_element(K key) {
    ...
    if (current != NULL && current->get_key() == key) {
        ...
        // Remove levels which have no elements(删除最高层的节点，层数-1)
